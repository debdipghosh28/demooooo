...
class RelivioESP32Hub {
    ...
    if (this.listeners[event]) {
      this.listeners[event].forEach((cb) => {
        try {
          ...
          console.error(`[ESP32Hub] Listener Error in ${event}:`, err);
        }
      });
    }
  }
  ...
  setStatus(status, mode = this.activeMode) {
    ...
    this.emit("status", {
      status,
      mode,
      deviceId: this.currentVitals.deviceId,
    });
  }
  ...
  async connectBLE() {
    ...
    try {
      ...
      this.bleDevice = await navigator.bluetooth
        .requestDevice({
          filters: [{ namePrefix: "Relivio" }, { namePrefix: "ESP32" }],
          optionalServices: [
      ...
      this.log(
        `Selected Device: ${this.bleDevice.name || "Unnamed BLE Device"} (${this.bleDevice.id})`,
        "success",
      );
      ...
      let service = null;
      try {
        service = await this.bleServer.getPrimaryService(
          this.BLE_CONFIG.serviceUuid,
        );
        this.log("Found Relivio Custom Vitals Service!", "success");
      } catch (e) {
        this.log(
          "Relivio service not found, trying Nordic UART Service...",
          "warn",
        );
        service = await this.bleServer.getPrimaryService(
          this.BLE_CONFIG.nusServiceUuid,
      ...
      try {
        this.bleTelemetryChar = await service.getCharacteristic(
          this.BLE_CONFIG.telemetryUuid,
        );
      } catch (e) {
        this.bleTelemetryChar = await service.getCharacteristic(
          this.BLE_CONFIG.nusTxUuid,
        );
      }

      await this.bleTelemetryChar.startNotifications();
      this.bleTelemetryChar.addEventListener(
      ...
      try {
        this.bleCommandChar = await service.getCharacteristic(
          this.BLE_CONFIG.commandUuid,
        );
      } catch (e) {
        try {
          this.bleCommandChar = await service.getCharacteristic(
            this.BLE_CONFIG.nusRxUuid,
          );
        } catch (err) { }
      }
      ...
      this.log(
        "BLE Connected! Receiving real-time vital telemetry.",
        "success",
      );
    } catch (error) {
  ...
  async connectBackendSerial(port, baudRate = 115200, board = "ESP32") {
    try {
      ...
      this.log(
        `Opening Server PySerial port ${port} at ${baudRate} baud (${board})...`,
        "info",
      );
  ...
  async connectSerial(baudRate = 115200, board = "ESP32") {
    if (!navigator.serial) {
      this.log(
        "Web Serial API is not supported in this browser. Falling back to Server Python Serial...",
        "warn",
      );
      throw new Error(
        'Web Serial API not supported in this browser. Please use the "Connect via Server (PySerial)" option below.',
      );
    }
    ...
    try {
      this.setStatus("connecting", "serial");
      this.log(
        `Select your ${board} USB COM Port in the browser dialog...`,
        "info",
      );
      ...
      this.setStatus("connected", "serial");
      this.log(
        `Web Serial Port opened at ${baudRate} baud! Streaming ${board} vitals...`,
        "success",
      );
  ...
  async readSerialStream() {
    ...
    const readableStreamClosed = this.serialPort.readable.pipeTo(
  ...
  /* ----------------------------------------------------------------------
       ...
       ---------------------------------------------------------------------- */
  connectWiFiSSE(preserveActiveMode = false) {
    const previousMode = this.activeMode;
    const previousStatus = this.connectionStatus;
    if (!preserveActiveMode) this.setStatus("connecting", "wifi");
    this.log(
      "Subscribing to Relivio Real-time IoT Hub SSE Stream (/api/esp32/stream)...",
      "info",
    );
    ...
    this.eventSource.onopen = () => {
      ...
      this.stats.startTime = Date.now();
      this.log(
        "Connected to Relivio WiFi IoT Hub! Ready to receive live ESP32 packets.",
        "success",
      );
    };
  ...
  startWiFiDirectPolling(esp32Ip, intervalMs = 1500) {
    ...
    this.log(
      `Starting direct poll on http://${this.wifiTargetIp}/vitals...`,
      "info",
    );
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (this.connectionStatus !== "connected") {
            this.setStatus("connected", "wifi");
            this.log(
              `Direct WiFi link established with ESP32 at ${this.wifiTargetIp}`,
              "success",
            );
          }
        }
  ...
  startSimulator(tickIntervalMs = 1000) {
    ...
    this.simInterval = setInterval(() => {
      ...
      const tempFluctuation =
        Math.sin(tick * 0.2) * 0.08 + (Math.random() - 0.5) * 0.04;
      const hrFluctuation = Math.round(
        Math.sin(tick * 0.3) * 2 + (Math.random() - 0.5) * 2,
      );
      ...
      const simData = {
        ...
        temperature: parseFloat(
          (this.simConfig.temp + tempFluctuation).toFixed(1),
        ),
        heart_rate: Math.max(
          45,
          Math.min(180, this.simConfig.hr + hrFluctuation),
        ),
        spo2: parseFloat(
          Math.min(
            100,
            Math.max(90, this.simConfig.spo2 + spo2Fluctuation),
          ).toFixed(1),
        ),
        humidity: parseFloat(
          (this.simConfig.humidity + (Math.random() - 0.5) * 0.5).toFixed(1),
        ),
  ...
  processIncomingPayload(raw, source) {
    ...
    if (cleaned.startsWith("[TX]"))
      cleaned = cleaned.replace("[TX]", "").trim();
      ...
      try {
        ...
        for (const pair of pairs) {
          ...
          if (k && v) {
            ...
            else if (lk.includes("hr") || lk.includes("heart"))
              obj.heart_rate = parseInt(v);
            else if (lk.includes("spo2") || lk.includes("ox"))
              obj.spo2 = parseFloat(v);
  ...
  processNormalizedData(data, source) {
    ...
    const temp = parseFloat(
      data.temperature || data.temp || this.currentVitals.temperature,
    );
    const hr = parseInt(
      data.heart_rate || data.hr || this.currentVitals.heartRate,
    );
    const spo2 = parseFloat(data.spo2 || this.currentVitals.spo2);
    const humidity = parseFloat(
      data.humidity || data.hum || this.currentVitals.humidity,
    );
    const aqi = parseInt(data.aqi || this.currentVitals.aqi);
    const bp =
      data.blood_pressure || data.bp || this.currentVitals.bloodPressure;
    ...
    const headache =
      data.headache || symptoms.headache || this.currentVitals.headache;
    const bodyAche =
      data.body_ache ||
      data.bodyAche ||
      symptoms.body_ache ||
      this.currentVitals.bodyAche;
    const fatigue =
      data.fatigue || symptoms.fatigue || this.currentVitals.fatigue;
    ...
    if (this.autoPredictEnabled) {
      const now = Date.now();
      if (now - this.lastAutoPredictTime > 3000) {
        // Debounce 3s
  ...
  syncVitalsToForm(vitals) {
    ...
    if (vitals.bloodPressure) {
      const bpRadio = document.querySelector(
        `input[name="Blood_Pressure"][value="${vitals.bloodPressure}"]`,
      );
    ...
    try {
      if (this.activeMode === "ble" && this.bleCommandChar) {
        ...
        const resp = await fetch("/api/esp32/command", {
          ...
          body: JSON.stringify({
            command: cmd,
            device_id: this.currentVitals.deviceId,
          }),
        });
        const res = await resp.json();
        if (res.success) {
  ...
  renderEcgFrame() {
    ...
    const hr =
      this.connectionStatus === "connected" ? this.currentVitals.heartRate : 72;

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnBackendSerialConnect = document.getElementById(
    "btnBackendSerialConnect",
  );
  ...
  function updateTempIndicator(tempC) {
    ...
    if (tempC < 37.3) {
      tempIndicator.innerHTML =
        '<span class="temp-state normal"><i class="fa-solid fa-circle-check"></i> Normal (&lt; 37.3°C)</span>';
    } else if (tempC <= 38.0) {
      tempIndicator.innerHTML =
        '<span class="temp-state mild"><i class="fa-solid fa-circle-dot"></i> Mild Fever (37.3°C - 38.0°C)</span>';
    } else {
      tempIndicator.innerHTML =
        '<span class="temp-state high"><i class="fa-solid fa-fire"></i> High Fever (&gt; 38.0°C)</span>';
    }
  }
  ...
  async function loadPresetCase(caseKey) {
    ...
    // Blood Pressure
    const bpRadio = document.querySelector(
      `input[name="Blood_Pressure"][value="${d.Blood_Pressure}"]`,
    );
    ...
    const smkRadio = document.querySelector(
      `input[name="Smoking_History"][value="${d.Smoking_History}"]`,
    );
    if (smkRadio) smkRadio.checked = true;

    const alcRadio = document.querySelector(
      `input[name="Alcohol_Consumption"][value="${d.Alcohol_Consumption}"]`,
    );
  ...
  predictionForm.addEventListener("submit", async (e) => {
    ...
    const formData = {
      ...
      Smoking_History: document.querySelector(
        'input[name="Smoking_History"]:checked',
      ).value,
      Alcohol_Consumption: document.querySelector(
        'input[name="Alcohol_Consumption"]:checked',
      ).value,
      ...
      Blood_Pressure: document.querySelector(
        'input[name="Blood_Pressure"]:checked',
      ).value,
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(
          `Recommended: ${result.prediction} (${result.confidence_percentage}%)`,
          "success",
        );
      } else {
  ...
  function renderPredictionResults(res) {
    ...
    dosageText.textContent =
      g.dosage_info || "Consult physician for standard dosing.";
    mechanismText.textContent =
      g.mechanism || "Standard antipyretic mechanism.";

    // Cautions
    if (g.precautions && g.precautions.length > 0) {
      cautionsContainer.classList.remove("hidden");
      cautionsList.innerHTML = g.precautions
        .map(
          (p) =>
            `<li><i class="fa-solid fa-triangle-exclamation"></i> ${p}</li>`,
        )
        .join("");
    } else {
    ...
    if (g.clinical_insights && g.clinical_insights.length > 0) {
      ...
      insightsList.innerHTML = g.clinical_insights
        .map((i) => `<li><i class="fa-solid fa-circle-check"></i> ${i}</li>`)
        .join("");
    }
  }
  ...
  async function refreshComPorts() {
    ...
    serialPortSelect.innerHTML =
      '<option value="">-- Scanning Windows COM Ports... --</option>';
    if (btnScanSerialPorts) {
      btnScanSerialPorts.innerHTML =
        '<i class="fa-solid fa-arrows-rotate fa-spin"></i> Scanning...';
    }
    ...
    try {
      ...
      if (!ports || ports.length === 0) {
        serialPortSelect.innerHTML =
          '<option value="">No COM ports detected (Check USB Cable/Driver)</option>';
      } else {
        ...
        ports.forEach((p) => {
          ...
          opt.value = p.port;
          const isBluetooth =
            p.board_hint && p.board_hint.includes("Bluetooth");
      ...
      serialPortSelect.innerHTML =
        '<option value="">Error scanning ports</option>';
    } finally {
      if (btnScanSerialPorts) {
        btnScanSerialPorts.innerHTML =
          '<i class="fa-solid fa-arrows-rotate"></i> Scan Ports';
      }
    }
  }
  ...
  if (closeEsp32ModalBtn)
    closeEsp32ModalBtn.addEventListener("click", closeModal);
  ...
  if (pillBoardEsp32 && pillBoardUno) {
    pillBoardEsp32.addEventListener("click", () => {
      ...
      window.esp32Hub.log(
        "Target board set: ESP32 Dev Module (115200 Baud)",
        "info",
      );
    });
  ...
  window.esp32Hub.on("status", ({ status, mode, deviceId }) => {
    ...
    // Header pill
    headerIotLabel.textContent = isConn
      ? `${selectedBoard}: ${mode.toUpperCase()} Active`
      : isConnecting
        ? "Connecting..."
        : `${selectedBoard}: Disconnected`;
    ...
    if (isConn) {
      ...
      hudConnectBtn.innerHTML =
        '<i class="fa-solid fa-sliders"></i> Hardware Controls';
    } else {
      ...
      hudProtocolBadge.innerHTML =
        '<i class="fa-solid fa-network-wired"></i> Ready';
      hudConnectBtn.innerHTML =
        '<i class="fa-solid fa-satellite-dish"></i> Connect Hardware';
    }

    // BLE Modal button states
    if (mode === "ble") {
      bleDot.className = `status-indicator-dot ${status}`;
      bleStatusText.textContent = isConn
        ? `Connected to ${deviceId}`
        : isConnecting
          ? "Connecting to ESP32 GATT..."
          : "Ready to pair with ESP32";
    ...
    if (mode === "serial") {
      ...
      if (serialStatusText) {
        serialStatusText.textContent = isConn
          ...
          : isConnecting
            ? `Opening COM Port for ${selectedBoard}...`
            : 'No COM Port Connected • Click "Scan Ports" below';
      }
      if (btnBackendSerialConnect)
        btnBackendSerialConnect.classList.toggle("hidden", isConn);
      if (btnSerialConnect) btnSerialConnect.classList.toggle("hidden", isConn);
      if (btnSerialDisconnect)
        btnSerialDisconnect.classList.toggle("hidden", !isConn);
    }
    ...
    // Simulator button
    if (mode === "simulator") {
      btnSimToggle.innerHTML = isConn
        ? '<i class="fa-solid fa-stop"></i> Stop Virtual Stream'
        : '<i class="fa-solid fa-play"></i> Start Virtual Stream';
  ...
  if (btnClearLog) {
    btnClearLog.addEventListener("click", () => {
      esp32TerminalBody.innerHTML =
        '<div class="terminal-line info"><span class="t-time">[00:00:00]</span> Diagnostic log cleared.</div>';
    });
  }
  ...
  if (btnBackendSerialConnect) {
    btnBackendSerialConnect.addEventListener("click", async () => {
      ...
      if (!port) {
        showToast(
          "Please select a COM port first (or click Scan Ports)",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        btnBackendSerialConnect.innerHTML =
        ...
        btnBackendSerialConnect.innerHTML =
          '<i class="fa-solid fa-plug"></i> Connect via Server (PySerial)';
      }
    });
  }
